...
genes.df <- getBM(mart = mart, 
      ...
      value = list("caelegprjna13758", rownames(emb.unsort1.data)),
...
c(nrow(genes.df), nrow(emb.unsort1.data))

# are there are genes missing?
nrow(emb.unsort1.data)-nrow(genes.df)
```
...
genes.df <- getBM(mart = mart, 
      filters = c("species_id_1010", "wbps_gene_id"),
      value = list("caelegprjna13758", rownames(emb.unsort2.data)),
      attributes = c('wbps_gene_id','external_gene_id', 'chromosome_name')
      )
...
c(nrow(genes.df), nrow(emb.unsort2.data))

# are there are genes missing?
nrow(emb.unsort2.data)-nrow(genes.df)
```
...
ggsave("./Output/unsort1/emb.unsort1.featurescatter.png")
```
...
ggsave("./Output/unsort2/emb.unsort2.featurescatter.png")
```
...
ggsave("./Output/GFP1/emb.GFP1.featurescatter.png")
```
...
ggsave("./Output/GFP2/emb.GFP2.featurescatter.png")
```
...
```{r}
emb.unsort1.markers %>% group_by(cluster) %>% filter(p_val_adj < .01) %>% top_n(n=2, wt=avg_log2FC) -> top2_unsort1

DoHeatmap(emb.unsort1, features = top2_unsort1$gene,raster=F)
ggsave("./Output/unsort1/emb.unsort1.HeatMap-top2.png")
```

```{r}
emb.unsort2.markers %>% group_by(cluster) %>% filter(p_val_adj < .01) %>% top_n(n=2, wt=avg_log2FC) -> top2_unsort2

DoHeatmap(emb.unsort2, features = top2_unsort2$gene,raster=F)
ggsave("./Output/unsort2/emb.unsort2.HeatMap-top2.png")
```

```{r}
emb.GFP1.markers %>% group_by(cluster) %>% filter(p_val_adj < .01) %>% top_n(n=2, wt=avg_log2FC) -> top2_GFP1

DoHeatmap(emb.GFP1, features = top2_GFP1$gene,raster=F)
ggsave("./Output/GFP1/emb.GFP1.HeatMap-top2.png")
```

```{r}
emb.GFP2.markers %>% group_by(cluster) %>% filter(p_val_adj < .01) %>% top_n(n=2, wt=avg_log2FC) -> top2_GFP2

DoHeatmap(emb.GFP2, features = top2_GFP2$gene,raster=F)
ggsave("./Output/GFP2/emb.GFP2.HeatMap-top2.png")
```


# Now try integration and mergeing UMAPs 
...
```{r}
top = DimPlot(emb.GFP1, reduction = "umap", label = "TRUE", label.box = TRUE)

bottom = DimPlot(emb.GFP2, reduction = "umap", label = "TRUE", label.box = TRUE)
```
