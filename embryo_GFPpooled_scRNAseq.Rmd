...
DimPlot(emb.GFPpooled_v2.finalfiltClusters, reduction = "umap", pt.size = 1, label = TRUE, label.size = 4) + theme(legend.text = element_text(size = 10))
dev.off()
...
FeaturePlot(emb.GFPpooled_v2.finalfiltClusters, reduction = "umap", features = c("myo-3", "pat-10", "mls-1"), ncol = 3, coord.fixed = TRUE)
dev.off()
...
# Step 2: Compute variance of each gene across clusters
gene_variances <- apply(avg_expr, 1, var)

# Step 3: Select top variable genes (e.g., top 50)
top_genes <- names(sort(gene_variances, decreasing = TRUE))[1:50]

# Step 4: Extract the expression values for top genes
heatmap_data <- avg_expr[top_genes, ]

# Step 5: Optional — scale genes (z-score across clusters)
