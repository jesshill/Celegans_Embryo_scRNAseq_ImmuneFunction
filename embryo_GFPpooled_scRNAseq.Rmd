...
levels(Idents(emb.GFPpooled_v2))

# find markers for every cluster compared to all remaining cells, report only the positive
emb.GFPpooled_v2.markers <- FindAllMarkers(emb.GFPpooled_v2, only.pos = F, min.pct = 0.25, logfc.threshold = 1)

table(Idents(emb.GFPpooled_v2))
table(Idents(emb.GFPpooled_v2), emb.GFPpooled_v2$cca_clusters) #the cca clusters were used for FindAllMarkers()
...
table(Idents(emb.GFPpooled_v2.filteredClustersFINAL))
...
### output this heatmap data
heatmap_genes <- unique(c(top60$gene, "cpr-1", "ifd-2", "ttr-52", "itr-1"))

heatmap_markers_export <- emb.GFPpooled_v2.filteredClustersFINAL.markers %>% filter(gene %in% heatmap_genes) %>% arrange(cluster, desc(avg_log2FC))

write.csv(heatmap_markers_export, file = "./Seurat_output/all_GFP_v2/emb.GFPpooled_v2.filteredClustersFINAL.HeatMap-top64.markerStats.csv", row.names = FALSE, quote = FALSE)
###
...
# "itr-1" is not posterior exclusive or intestine exclusive 
```
...
# Inspect current cluster names - can refactor to order (early, mid, late development)
levels(bp@compareClusterResult$Cluster)

# Set desired order
bp@compareClusterResult$Cluster <- factor(
  bp@compareClusterResult$Cluster,
  levels = c("12","7","1","4","9","5","11","3","0","6","10","8","2"))
...
bp.background_df <- bp_df
str(bp.background_df) 
...
bp.background_df_filtered <- bp.background_df %>% filter(str_detect(bp.background_df$Description, "immune*|innate*|defense*"))

bp.all_df_filtered <- bp.all_df %>% filter(str_detect(bp.all_df$Description, "immune*|innate*|defense*"))

str(bp.background_df_filtered)
dim(bp.background_df_filtered)

str(bp.all_df_filtered)
dim(bp.all_df_filtered)

# expand the gene ID column 
genes.all <- bp.all_df_filtered %>% separate_rows(geneID, sep = "/")  
dim(genes.all) # 2316   13
str(genes.all)  
str(genes.all$geneID)


genes.background <- bp.background_df_filtered %>% separate_rows(geneID, sep = "/")  
...
genes.all.all <- bp.all_df %>% separate_rows(geneID, sep = "/")  
genes.background.all <- bp.background_df %>% separate_rows(geneID, sep = "/")  
...
# bp_df, this variable was made up above

gene_df <- bp_df %>% separate_rows(geneID, sep = "/")
dim(gene_df)

clus.0 <- gene_df[gene_df$Cluster == "0", ]
clus.1 <- gene_df[gene_df$Cluster == "1", ]
clus.2 <- gene_df[gene_df$Cluster == "2", ]
clus.3 <- gene_df[gene_df$Cluster == "3", ]
clus.4 <- gene_df[gene_df$Cluster == "4", ]
clus.5 <- gene_df[gene_df$Cluster == "5", ]
clus.6 <- gene_df[gene_df$Cluster == "6", ]
clus.7 <- gene_df[gene_df$Cluster == "7", ]
clus.8 <- gene_df[gene_df$Cluster == "8", ]
clus.9 <- gene_df[gene_df$Cluster == "9", ]
clus.10 <- gene_df[gene_df$Cluster == "10", ]
clus.11 <- gene_df[gene_df$Cluster == "11", ]
clus.12 <- gene_df[gene_df$Cluster == "12", ]
...
## different upset plot: Genes shared across clusters for each GO term

# shared GO terms across clusters only 
shared_terms <- gene_df %>% group_by(Description) %>% summarise(n_clusters = n_distinct(Cluster)) %>% filter(n_clusters > 1)
dim(shared_terms) # 52 2

# extract genes per GO term per cluster
term_cluster_genes <- gene_df %>% filter(Description %in% shared_terms$Description) %>% group_by(Description, Cluster) %>% summarise(genes = list(unique(geneID)), .groups = "drop")
dim(term_cluster_genes) # 146 3

# find shared genes within each GO term
library(purrr)

# shared_gene_results <- term_cluster_genes %>% group_by(Description) %>% 
#   summarise(shared_genes = purrr::reduce(genes, intersect), n_shared = length(purrr::reduce(genes, intersect)), clusters = list(unique(Cluster)), .groups = "drop") %>% filter(n_shared > 0)

shared_gene_results <- term_cluster_genes %>% group_by(Description) %>% summarise(shared_genes = list(unique(unlist(genes))), clusters = list(unique(Cluster)), .groups = "drop") %>% mutate(n_shared = lengths(shared_genes)) %>% filter(n_shared > 0)

shared_gene_results

shared_go_terms <- shared_gene_results$Description

gene_df_shared <- gene_df %>%
  filter(Description %in% shared_go_terms)

go_filtered <- shared_gene_results %>%
  filter(n_shared >= 3) %>%   # increase threshold now that sets are bigger
  arrange(desc(n_shared)) %>%
  slice_head(n = 8)

go_list <- go_filtered$shared_genes
names(go_list) <- go_filtered$Description

upset(fromList(go_list),
  nsets = length(go_list))




# build GO term gene list
go_list <- shared_gene_results$shared_genes
names(go_list) <- shared_gene_results$Description

# filter to strongest shared 
go_filtered <- shared_gene_results %>% dplyr::filter(n_shared >= 3) 

go_list <- go_filtered$shared_genes
names(go_list) <- go_filtered$Description

# top GO terms by number of shared genes
go_filtered <- go_filtered %>% dplyr::arrange(desc(n_shared)) %>% dplyr::slice_head(n = 15)
...
posterior_clusters <- as.character(c(0,6,10))
...
# Output of the scaled data (z-scored expression values) used in the variance heatmap. Exported as a .csv.
write.csv(scaled_data, file = "./Seurat_output/all_GFP_v2/emb.GFPpooled_v2.filteredClustersFINAL.varianceHM.scaled.csv", quote = FALSE)
```
...
```{r}
pdf("./Seurat_output/all_GFP_v2/emb.GFPpooled_v2.filteredClustersFINAL.featureplot.immune-interests.pdf", height = 6, width = 15)
FeaturePlot(emb.GFPpooled_v2.filteredClustersFINAL, reduction = "umap.filtered.final", features = c("drh-1","sta-1", "zip-2", "ogt-1", "oga-1"), ncol = 3, coord.fixed = TRUE)
dev.off()


pdf("./Seurat_output/all_GFP_v2/emb.GFPpooled_v2.filteredClustersFINAL.featureplot.unc-82.pdf", height = 6, width = 15)
FeaturePlot(emb.GFPpooled_v2.filteredClustersFINAL, reduction = "umap.filtered.final", features = c("unc-82"), ncol = 3, coord.fixed = TRUE)
dev.off()

emb.GFPpooled_v2.filteredClustersFINAL.markers %>% group_by(cluster) %>% filter(grepl("clec*",gene), p_val_adj < .01) %>% top_n(n=3, wt=avg_log2FC) -> top3clecs
pdf("./Seurat_output/all_GFP_v2/emb.GFPpooled_v2.filteredClustersFINAL.HeatMap.clecs.pdf", height = 6, width = 15)
DoHeatmap(emb.GFPpooled_v2.filteredClustersFINAL, features = top3clecs$gene,raster=F)
dev.off()

pdf("./Seurat_output/all_GFP_v2/emb.GFPpooled_v2.filteredClustersFINAL.featureplot.clecs.pdf", height = 15, width = 15)
FeaturePlot(emb.GFPpooled_v2.filteredClustersFINAL, reduction = "umap.filtered.final", features = c("clec-167", "clec-266", "clec-40", "clec-47", "clec-38", "clec-36", "clec-37", "clec-35", "clec-31", "clec-33", "clec-32", "clec-28", "clec-25", "clec-22", "clec-26", "clec-27"), ncol = 3, coord.fixed = TRUE)
dev.off()


pdf("./Seurat_output/all_GFP_v2/emb.GFPpooled_v2.filteredClustersFINAL.featureplot.clecs-examples.pdf", height = 6, width = 15)
FeaturePlot(emb.GFPpooled_v2.filteredClustersFINAL, reduction = "umap.filtered.final", features = c("clec-56", "clec-37"), ncol = 3, coord.fixed = TRUE)
dev.off()
```
...
```{r}
# view data identities/levels 
emb.GFPpooled_v2.filteredClustersFINAL
emb.GFPpooled_v2.filteredClustersFINAL$cell_timing
emb.GFPpooled_v2.filteredClustersFINAL$cell_region
table(Idents(emb.GFPpooled_v2.filteredClustersFINAL))
Idents(emb.GFPpooled_v2.filteredClustersFINAL)

# create a new column in our seurat object
emb.GFPpooled_v2.filteredClustersFINAL$cell_region <- as.character(Idents(emb.GFPpooled_v2.filteredClustersFINAL))
emb.GFPpooled_v2.filteredClustersFINAL$cell_timing <- as.character(Idents(emb.GFPpooled_v2.filteredClustersFINAL))

emb.GFPpooled_v2.filteredClustersFINAL$cell_timing[
  emb.GFPpooled_v2.filteredClustersFINAL$filtered_clusters_FINAL %in% c("12")
] <- "early_dev"

emb.GFPpooled_v2.filteredClustersFINAL$cell_timing[
  emb.GFPpooled_v2.filteredClustersFINAL$filtered_clusters_FINAL %in% c("1", "4", "7")
] <- "mid_dev"

emb.GFPpooled_v2.filteredClustersFINAL$cell_timing[
  emb.GFPpooled_v2.filteredClustersFINAL$filtered_clusters_FINAL %in% c("0", "3", "5", "6", "9", "10", "11")
] <- "late_dev"


emb.GFPpooled_v2.filteredClustersFINAL$cell_region[
  emb.GFPpooled_v2.filteredClustersFINAL$filtered_clusters_FINAL %in% c("3", "4", "5", "9")
] <- "anterior"

emb.GFPpooled_v2.filteredClustersFINAL$cell_region[
  emb.GFPpooled_v2.filteredClustersFINAL$filtered_clusters_FINAL %in% c("0", "6")
] <- "central"

emb.GFPpooled_v2.filteredClustersFINAL$cell_region[
  emb.GFPpooled_v2.filteredClustersFINAL$filtered_clusters_FINAL %in% c("10")
] <- "posterior"


  
cell_annotations <- data.frame(
  cell_id = colnames(emb.GFPpooled_v2.filteredClustersFINAL),
  cluster = emb.GFPpooled_v2.filteredClustersFINAL$filtered_clusters_FINAL,
  cell_region = emb.GFPpooled_v2.filteredClustersFINAL$cell_region,
  cell_timing = emb.GFPpooled_v2.filteredClustersFINAL$cell_timing
)

write.csv(cell_annotations, file = "./Seurat_output/all_GFP_v2/emb.GFPpooled_v2.filteredClustersFINAL.cell_annotations.csv", row.names = FALSE)
write.csv(cell_annotations, file = "./Seurat_output/all_GFP_v2/emb.GFPpooled_v2.filteredClustersFINAL.cell_annotations.csv")
