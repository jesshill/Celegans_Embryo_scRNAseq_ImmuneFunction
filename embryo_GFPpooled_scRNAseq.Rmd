...
# Load the data
```{r}
emb.GFPrep1and2.data <- Read10X_h5("./cellranger/GFP_rep1and2/filtered_feature_bc_matrix.h5")

emb.GFPrep3and4.data <- Read10X_h5("./cellranger/GFP_rep3and4/filtered_feature_bc_matrix.h5")

#emb.GFPpooled.data <- Read10X_h5("./cellranger/all_GFP_v1/outs/count/filtered_feature_bc_matrix.h5")
```
...
genes.df <- getBM(mart = mart, 
      ...
      value = list("caelegprjna13758", rownames(emb.GFPrep1and2.data)),
...
c(nrow(genes.df), nrow(emb.GFPrep1and2.data))

# there are genes missing
nrow(emb.GFPrep1and2.data)-nrow(genes.df)
...
genes1.df <- getBM(mart = mart, 
      filters = c("species_id_1010", "gene_name"),
      value = list("caelegprjna13758", rownames(emb.GFPrep3and4.data)),
      attributes = c('wbps_gene_id','external_gene_id', 'chromosome_name'))

mitoList = genes1.df[genes1.df$chromosome_name == 'MtDNA', 'external_gene_id']
genes1.df$chromosome_name = NULL

c(nrow(genes1.df), nrow(emb.GFPrep3and4.data))

# there are genes missing
nrow(emb.GFPrep3and4.data)-nrow(genes1.df)
```
...
```{r}
emb.GFPrep1and2.data.filt <- emb.GFPrep1and2.data[rownames(emb.GFPrep1and2.data) %in% genes.df$external_gene_id,]

# The two data structures do not have the same order of WBGeneIDs
identical(rownames(emb.GFPrep1and2.data.filt), genes.df$external_gene_id)

# Use match to make sure they are in the same order
identical(rownames(emb.GFPrep1and2.data.filt),
          genes.df[match(rownames(emb.GFPrep1and2.data.filt), genes.df$external_gene_id),]$external_gene_id)
...
# are there NAs?
sum(!is.na(rownames(emb.GFPrep1and2.data.filt)))

head(emb.GFPrep1and2.data.filt)



emb.GFPrep3and4.data.filt <- emb.GFPrep3and4.data[rownames(emb.GFPrep3and4.data) %in% genes1.df$external_gene_id,]

# The two data structures do not have the same order of WBGeneIDs
identical(rownames(emb.GFPrep3and4.data.filt), genes1.df$external_gene_id)

# Use match to make sure they are in the same order
identical(rownames(emb.GFPrep3and4.data.filt),
          genes1.df[match(rownames(emb.GFPrep3and4.data.filt), genes1.df$external_gene_id),]$external_gene_id)
...
# are there NAs?
sum(!is.na(rownames(emb.GFPrep3and4.data.filt)))

head(emb.GFPrep3and4.data.filt)
```
...
```{r}
emb.GFPrep1and2 <- CreateSeuratObject(counts = emb.GFPrep1and2.data.filt, project = "emb.GFPrep1and2", min.cells = 3, min.features = 200)

view(emb.GFPrep1and2@meta.data) #955 rows


emb.GFPrep3and4 <- CreateSeuratObject(counts = emb.GFPrep3and4.data.filt, project = "emb.GFPrep3and4", min.cells = 3, min.features = 200)

view(emb.GFPrep3and4@metas.data) #1560 rows
```

```{r}
length(unique(genes.df$wbps_gene_id))
length(unique(rownames(emb.GFPrep1and2)))
rownames(emb.GFPrep1and2)[!(rownames(emb.GFPrep1and2) %in% genes.df$external_gene_id)]


length(unique(genes1.df$wbps_gene_id))
length(unique(rownames(emb.GFPrep3and4)))
rownames(emb.GFPrep3and4)[!(rownames(emb.GFPrep3and4) %in% genes1.df$external_gene_id)]
```
...
view(emb.GFPrep1and2@meta.data)
head(emb.GFPrep1and2@meta.data, 5)

view(emb.GFPrep3and4@meta.data)
head(emb.GFPrep3and4@meta.data, 5)
```

Mitochondrial genome percentage 
```{r}
mitoIndices = which(rownames(emb.GFPrep1and2) %in% mitoList)
emb.GFPrep1and2[["percent.mt"]] <- PercentageFeatureSet(emb.GFPrep1and2, features = mitoIndices)


mitoIndices = which(rownames(emb.GFPrep3and4) %in% mitoList)
emb.GFPrep3and4[["percent.mt"]] <- PercentageFeatureSet(emb.GFPrep3and4, features = mitoIndices)
```

Visualize QC metrics as a violin plot
```{r}
VlnPlot(emb.GFPpooled, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
ggsave("./Seurat_output/all_GFP_v1/emb.GFPpooled.VlnPlot.png")
...
plot1 + plot2
ggsave("./Seurat_output/all_GFP_v1/emb.GFPpooled.FeatureScatter.png")
```
...
ggsave("./Seurat_output/all_GFP_v1/emb.GFPpooled.VlnPlot.filt.png")

FeatureScatter(emb.GFPpooled, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
ggsave("./Seurat_output/all_GFP_v1/emb.GFPpooled.FeatureScatter.filt.png")
```
...
ggsave("./Seurat_output/all_GFP_v1/emb.GFPpooled.VarFeatures.png")
```
...
DoHeatmap(emb.GFPpooled, features = c(top2$gene, "clec-56", "pbo-4", "C14C6.5"), raster=F) + 
