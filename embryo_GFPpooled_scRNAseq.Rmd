...
ggsave("./Output/GFPpooled/emb.GFPpooled.vlnplot.png")
```
...
```{r}
emb.GFPpooled.markers <- FindAllMarkers(emb.GFPpooled, only.pos = T, min.pct = 0.25, logfc.threshold = 1.4) # 0.25 is the default threshold 
...
write.csv(emb.GFPpooled.markers,"./Output/GFP_pooled/emb_GFPpooled_markers_unthresholded.csv", row.names = TRUE)
```
...
my_ontology_plot(my_tea_analysis(emb.GFPpooled.markers %>% filter(p_val_adj < 0.05, avg_log2FC > 1.5), tissue_df), 
                 "Tissue Ontology")
```
...
write.csv(emb.GFPpooled.markers.clecs,"./Output/emb_GFPpooled_marker_clecs.csv", row.names = TRUE)
```
...
```{r}
emb.GFPpooled.specificClusters <- RunUMAP(subset(emb.GFPpooled, idents = c('1','11','12', '13'), invert = TRUE), dims = 1:10)
