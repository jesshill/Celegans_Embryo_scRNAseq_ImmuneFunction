...
```{r}
emb_int.markers <- FindAllMarkers(emb_int, only.pos = T, min.pct = 0.25, logfc.threshold = 1.4) # 0.25 is the default threshold 
...
write.csv(emb_int.markers,"./Output/GFP_pooled/emb_int_markers.csv", row.names = TRUE)
```
...
emb_int.markers %>% group_by(cluster) %>% filter(p_val_adj < .01) %>% top_n(n=2, wt=avg_log2FC) -> top2

DoHeatmap(emb_int, features = top2$gene,raster=F)


FeaturePlot(emb_int, features = c("elt-2","elt-7", 'ges-1'),raster=F)
#VlnPlot(emb_int, features = c("elt-2","elt-7", 'ges-1'))

FeaturePlot(emb_int, features = c("glh-1","chs-1", "myo-2", "myo-3", "hlh-2", "rab-3"), raster=F)
VlnPlot(emb_int, features = c("chs-1", "myo-2", "myo-3", "elt-2", "ges-1"))
```
