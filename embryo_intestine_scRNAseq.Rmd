...
```{r}
emb_int.markers1 <- FindAllMarkers(emb_int, only.pos = F, min.pct = 0.25, logfc.threshold = 0.25)

emb_int.markers1 %>% 
  group_by(cluster) %>%
  slice_max(n = 2, order_by = avg_log2FC)

emb_int.markers2 <- FindAllMarkers(emb_int, only.pos = F, min.pct = 0.25, logfc.threshold = 1)
...
write.csv(emb_int.markers,"./emb_int_markers.csv", row.names = TRUE)
```
